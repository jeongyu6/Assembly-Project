...
75, 44, 5, 5, TYPE_PICKUP  
105, 50, 5, 5, TYPE_PICKUP
15, 25, 5, 5, TYPE_PICKUP
120, 55, 5, 5, TYPE_PICKUP
...
b_posy_old: .word 0
b_dy:   .word 0
on_plat: .word 0  # 0 false, 1 stationary, 2 moving
health: .word 1
...
normal_box:
	...
	beq    $s4, $s5, label1      # is not on current platform still 0 or still 1
	bne    $s1, $s3, label1      # if (i != moving index) don't move bird
...
update_moving_platform:
	...
	li     $t5, 65
	blt    $t1, $t5, reverse_moving_dir
	add    $t6, $t1, $t2
	li     $t7, 90
	bgt    $t6, $t7, reverse_moving_dir
...
update_enemy:
	...
	# Clamp between 90 and 120 in terms of the x direction
...
# Return:
# $v0 (1 if collision, 0 is no collision)
#-----------------------------------#
...
#Debugging here
# Special case for pickups 
# Check if bird is standing on pickup 
sub     $t8, $t4, $a1     # bird_bottom - obstacle_y
blt     $t8, 0, not_on_platform
bgt     $t8, 1, not_on_platform

# Confirm horizontal overlap
blt     $t3, $a0, not_on_platform  # bird_right < obs_left
bgt     $t0, $t2, not_on_platform  # bird_left > obs_right

# We'll treat it as a collision
li      $v0, 5  # Unique value for standing-on-top-of-pickup
jr      $ra
